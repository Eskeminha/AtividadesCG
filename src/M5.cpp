/* Módulo 5 - Câmera Sintética em Primeira Pessoa
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT,
        "Modulo 5 - Camera -- Anderson Koefender!", nullptr, nullptr);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        renderText(string("1 - Key Light:  ") + (lightOn[0] ? "ON" : "OFF"), 10, 56, s, lightOn[0] ? corOn : corOff);
        renderText(string("2 - Fill Light: ") + (lightOn[1] ? "ON" : "OFF"), 10, 74, s, lightOn[1] ? corOn : corOff);
        renderText(string("3 - Back Light: ") + (lightOn[2] ? "ON" : "OFF"), 10, 92, s, lightOn[2] ? corOn : corOff);

        float yBase = HEIGHT - 100.0f;
        renderText("WASD  : mover camera",        10, yBase,      s, glm::vec3(0.75f, 0.75f, 0.75f));
        renderText("Mouse : rotacionar camera",   10, yBase + 18, s, glm::vec3(0.75f, 0.75f, 0.75f));
        renderText("TAB   : proximo objeto",       10, yBase + 36, s, glm::vec3(0.75f, 0.75f, 0.75f));
        renderText("R + X/Y/Z : rotacionar obj",  10, yBase + 54, s, glm::vec3(0.75f, 0.75f, 0.75f));
